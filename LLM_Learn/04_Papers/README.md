# Classic AI & Robotics Papers

## 定位

这里是长期论文库，不是临时阅读队列。

每天 `start-my-day` 可以从这里挑一篇经典 AI / Robotics / Robot Learning 论文或材料，作为白天碎片时间的 `paper slot`。默认目标不是完整精读，而是形成：

- 一句话 takeaway
- 一个和当前 mini-stack / 职业能力的连接
- 一个后续可验证的 idea

只有当论文进入当前 Phase 主线时，才升级成精读和代码 / 实验复现。

## 目录结构

- `00_Reading_Workflow.md`：每天怎么读、读到什么程度、如何回写 Daily
- `01_Reading_Index.md`：当前论文队列、分类和阅读顺序
- `10_AI_Foundations/`：AI 基础经典思想
- `20_Robot_Learning/`：IL / RL / manipulation policy
- `30_VLA_and_Foundation_Policies/`：VLA / generalist robot policy
- `40_Data_and_Eval/`：机器人数据、数据集、评测和闭环

## start-my-day 使用规则

- 周一到周四：默认安排 1 篇论文的 20-40m paper slot
- 周五：默认做论文 catch-up / takeaway 汇总
- 周末：只有主线任务完成后，才安排论文精读

paper slot 不抢晚上主学习时间。晚上主学习仍然服务当前 Weekly / Monthly 的硬产出。

