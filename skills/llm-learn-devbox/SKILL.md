...
description: Work with the dedicated learning development machine and remote experiment repos used alongside the LLM_Learn workspace. Use when the task involves ssh dev1, remote environment discovery, GPU/CUDA/Python/PyTorch setup, robotics / MuJoCo / LeRobot / CV / LLM experiments, or coordinating local notes with remote experiment code and runs.
---
...
- `nano-vllm`: inference/runtime oriented repo, relevant when the current task is on the language intelligence support line

When the user is focused on inference / runtime, prefer starting from:
- `~/llm_learn/nano-vllm`
