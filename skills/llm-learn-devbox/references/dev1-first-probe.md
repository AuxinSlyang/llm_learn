...
- `~/llm_learn/micrograd` and `~/llm_learn/makemore` are foundational / pedagogical
- `~/llm_learn/nano-vllm` is relevant when the current task is on the language intelligence / inference-runtime support line

## Practical default move
If the task is about inference / runtime and needs remote execution:
1. `ssh dev1`
2. `llm`
